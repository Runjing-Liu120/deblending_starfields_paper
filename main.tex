...
\if1\blind
{
  ...
  \author{Runjing Liu
  %\thanks{The authors gratefully acknowledge \textit{please remember to list all relevant funding sources in the unblinded version}. NSF graduate research fellowship etc ... }
  \hspace{.2cm}\\
...
Deblending is the task of distinguishing and characterizing individual light sources.
We propose a new, fully Bayesian method to deblend astronomical images of crowded star fields called StarNet.
...
We illustrate that Wake-Sleep, an algorithm which minimizes forward KL divergence, has significant benefits compared to traditional variational inference, which minimizes a reverse KL divergence.
In our experiments with SDSS images of the M2 globular cluster, StarNet is substantially more accurate than two competing methods: Probablistic Cataloging (PCAT), a method that uses MCMC for inference, and a second method based on the traditional software pipeline used in deblending (DAOPHOT).
