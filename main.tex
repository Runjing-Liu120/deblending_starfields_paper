...
\bigskip
\begin{abstract}
A key step in constructing catalogs from astronomical surveys is deblending, the task of identifying and characterizing each imaged light source.
In crowded star fields, light sources often cannot be unambiguously separated. 
Probabilistic cataloging defines a joint distribution on raw telescope images and their catalogs; ambiguities in catalog construction 
are captured by the Bayesian posterior.
However, current probabilistic cataloging methods typically employ MCMC (Markov chain Monte Carlo),
whose computational cost is prohibitive for the terabytes of data collected by modern astronomical surveys. 
...
However, the wake-sleep algorithm 
fits the approximate posterior by optimizing the  forward KL (Kullback-Leibler) divergence instead of the traditionally used reverse KL divergence.
We find that the the wake-sleep algorithm produces more reliable catalogs by avoiding shallow local minima in the reverse KL. 
StarNet outperforms existing cataloging routines on the crowded starfield M2. In addition, StarNet is $10^5$ times faster than MCMC and exhibits the scaling characteristics necessary for inference on large astronomical surveys. 
