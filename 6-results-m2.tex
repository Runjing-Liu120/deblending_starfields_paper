...
in this subimage, the HST catalog contains 1114 stars with F606W-band magnitudes less than 22.
...
We compute the StarNet credible intervals by sampling from the variational posterior on each tile;
recall that on each tile, the variational posterior on the number of stars is a categorical random variable.
The the total number of inferred stars in the image is the sum of the number of star in each tile.
