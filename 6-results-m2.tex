...
which has greater resolution than the Sloan telescope (0.05 and 0.4 arcseconds per pixel, respectively; see~\cite{hubble_about} and~\cite{sdss_about}). The reported catalog from this Hubble survey serves as the ground truth for validating our results.
...
We include two bands in our analysis, the SDSS $r$-band and $i$-band. 
...
There are 1114 stars in the Hubble catalog.
...
The improvement of StarNet-WS in PPV was most pronounced for dim stars (Figure~\ref{fig:summary_stats}). The TPR for StarNet-WS was uniformly better than DAOPHOT across all magnitudes. 
Of all methods, StarNet-WS best approximated the Hubble flux distribution (Figure~\ref{fig:luminosity_fun_m2}). 
...
\begin{figure}[tb]
    ...
    \vspace{-0.4cm}
    \caption{Source magnitude histograms for the $r$-band observations of M2. 
    ...
    For PCAT, the magnitude histogram is from a single catalog sample. }
...
Appendix~\ref{sec:test_m2} presents these results of StarNet, PCAT, and DAOPHOT on a neighboring $100 \times 100$ pixel subimage of M2. 
