...
stars. The catalog specifies the number of stars 
in the image, and for each star, the catalog 
records its location, brightness, and color. 
...
\jeff{Sounds good. I really like defining a catalog as a sample, not a distribution. If Portillo defines it this way too, perhaps we can make the new definition stick.}
the location of the $i$th star is $\ell_i \in \Reals^2$;
and the flux in the $b$th image band of the $i$th star is $f_{i, b}\in\Reals^+$. 

The prior is a distribution over the set of catalogs $\Theta$. In this work, we use a marked spatial Poisson process. That is, we first draw the number of stars contained in the $H\times W$ image:
\begin{align}
