Stars radiate photons. A telescope image records the number of photons that arrive at each pixel. Telescope images also consist of multiple filter bands, each absorbing photons within a specified range of wavelengths. 
...
The model parameters $\pi$ and $\beta$ are estimated by the SDSS software pipeline and reported along with the release of each SDSS image. Section~\ref{sec:model_params} introduces an alternative method that estimates these parameters jointly with the approximate posterior. 
