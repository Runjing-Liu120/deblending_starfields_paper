...
To evaluate the $-\log q_\eta(z | x)$ for some $(x,z)\sim p$, first convert $z$ to its tile parameterization $(\tilde N^{(s,t)}, \tilde \ell^{(s,t)}, \tilde f^{(s,t)})_{s=1,t=1}^{(S,T)}$, as detailed in Appendix REF.
...
\end{align}
$\omega^{(s,t)}_n$ is the output of the neural network, and \eqref{eq:cross_entropy_loss} is the usual cross-entropy loss for a multi-class classification problem. 

Next, recall that location coordinates are logit-normal and fluxes are log-normal. 
Let $y$ generically denote either the logit-location or log-flux for a star in catalog $z$; let $(\mu, \sigma)$ be the Guassian mean and standard deviation returned by the neural network. Then,
\begin{align}
    -\log q_\eta(y | x) = 
...
\end{align}
The first term encourages network predictions $\mu$ to be close to the sampled latent variable $y$, while $\sigma^2$ encodes the uncertainty of the network: the second term encourages small uncertainties, but this is 
