The central quantity in Bayesian statistics is the posterior distribution $p(z|x)$; for our model, the posterior is the distribution of catalogs given an observed image $x$. 
...
in that $\eta^*$ is chosen according to~\eqref{eq:kl_objective}. In this case, $q_\eta(z | x)$ is usually written $q_\eta(z)$, suppressing the dependence on $x$. For a new observation $\tilde x$, the optimization problem~\eqref{eq:kl_objective} must be solved again for $\tilde x$ to find an 
...
Our factorization instead has a spatial structure. We partition the full $H \times W$ image into disjoint $R \times R$ tiles. 
...
\begin{figure}[!tb]
    ...
    \vspace{-0.5cm}
    \caption{The neural network returning parameters for the variational distribution on each $2 \times 2$ image tile. For cataloging M2, the input image is an $8\times 8$ padded tile, and the network is responsible for returning variational parameters for the center $2\times 2$ tile.\\
    \color{red}{Maybe you could rotate the vertical words 45 degrees so they are more readable?}\\
    \color{red}{$\omega$ etc aren't variational parameters}
    }
