A central quantity in Bayesian statistics is the posterior distribution. 
...
In {\itshape amortized} variational inference
...
Our factorization has a spatial structure. We decompose the full $H \times W$ image into disjoint $R \times R$ tiles. Each tile $\tilde x_{st}$ is composed of pixels:
\begin{align}
...
If $\tau$ is the mapping in~\eqref{eq:patch_to_full_map}, then the variational distribution on catalogs $\theta$ is
\begin{align}
...
% on the full image catalog $\theta$ as 
% \begin{align}
...
we need to evaluate $\tilde q_\eta(\tau^{-1}(\theta) | x)$. 
...
To evaluate the probability of $\tau^{-1}(\theta)$ under $\tilde q_\eta$, we must marginalize over the rows of the triangular arrays $\ell^{(s, t)}$ and $\tilde f^{(s, t)}$ that are not determined by $\theta$. However, 
...
to compute $q_\eta(\theta | x)$, it suffices to evaluate $\tilde q_\eta$ only at the rows of triangular 
...
% \begin{align}
%     \theta &\stackrel{d}{=} \tau\Big( \big(\tilde N^{(s, t)}, \tilde \ell^{(s, t)}, \tilde f^{(s, t)}\big)_{r=1, t = 1}^{s, t}\Big), \notag \\  
%         &\text{where } 
...
$\tilde q_\eta\big(\tilde N^{(s, t)}, \tilde \ell^{(s, t)}, \tilde f^{(s, t)} | x\big)$. On each tile, the distributional parameters are given by the output of a neural network. 


Dropping the index 
$(s,t)$ in this subsection, 
% \begin{align}
%     \tilde N &\sim \text{Categorical}\Big(
%     \omega_{\eta}(h^{(s,t)}) ; 0, ..., N_{max}\Big) \label{eq:var_distr_n}\\
% 	\tilde \ell_{N, i} / S &\sim \text{LogitNormal}\Big([u_{\eta}(h^{(s,t)})]^{\mu}_{N,i} , [u_{\eta}(h^{(s,t)})]^{\sigma^2}_{N,i} \Big) \label{eq:var_distr_loc}\\
% 	\tilde f^b_{N, i} &\sim \text{LogNormal}\Big([v_{\eta}(h^{(s,t)})]^{\mu}_{N,i} , [v_{\eta}(h^{(s,t)})]^{\sigma^2}_{N,i}\Big) \label{eq:var_distr_f}
% \end{align}
