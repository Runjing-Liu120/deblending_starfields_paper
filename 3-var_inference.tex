In the previous section, we have detailed the likelihood $p(x | z)$ which describes the generation of an image $x$ given a catalog $z$. We also specified a prior distribution $p(z)$ over the space 
...
\begin{align}
    ...
    \label{eq:tiles}
\end{align}
Let $S = \frac{H}{R}$ and $T = \frac{W}{R}$ and we assume that $H$ and $W$ are multiples of $R$. The tiles cover the image, so $s$ and $t$ range over $s = 1, ..., S$ and $t = 1, ..., T$. See Figure~\ref{fig:ex_tiles} for a schematic.
...
The architecture is fairly standard (Figure~\ref{fig:starnet_arch}), consisting of several convolutional layers followed by a series of fully connected layers. Convolutional layers are useful in our problem of localizing stars as they make the network invariant to shifts in stellar location. The size of the convolutional kernel in the first layer is taken to be three pixels, roughly the full width at half maximum (FWHM) of the PSF. The optimization of the architecture is left for future work; our primary contribution in this paper is the application of neural networks to provide a variational posterior for cataloging starfields, not the network archetecture per se. 
...
We emphasize that while the variational distribution factorizes over $2 \times 2$ tiles, we are not breaking the inference problem on the full image into embarrassingly parallel subproblems. We always evaluate the likelihood (example, when computing the ELBO in~\eqref{eq:elbo}) on the full image. Light from a star within a 2 x 2 tile spills over into neighboring tiles, so the likelihood does not decouple across image tiles. 
