...
\jeff{``Section'' is typically capitalized. Same for ``Figure''.}
...
\jeff{``Equation'' is usually capitalized. However, if you use eqref rather than ref (which doesn't add parentheses), it seems to be more common to omit the word ``Equation'' entirely.}
tractable in most 
...
\jeff{Maybe better to use parentheses around the superscripts so they don't look like exponents?}
...
In summary, 
\jeff{Better to say it once clearly than to have a summary afterwards.
Can you integrate the text below into the text before it?
}
...
\jeff{Is there another problem we address too?}
