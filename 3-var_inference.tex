...
In our model, the latent variable space is high-dimensional: it is the set of all catalogs. Therefore, approximate methods such as MCMC~\cite{Portillo_2017} or variational inference~\cite{Blei_2017_vi_review, Jordan_intro_vi, Wainwrite_graph_models_vi} are required. 
...
Minimizing the $\KL$ divergence in~\eqref{eq:kl_objective} is equivalent to maximizing the evidence lower bound, or ELBO~\cite{Blei_2017_vi_review},
\begin{align}
...
Note that the generative model in Section~\ref{sec:gen_model} does not assume any hierarchical structure over replicated  images. Therefore, given a new image $x^{new}$, the posterior factorizes; in other words,
...
The full latent variable of interest is $z = \{N, (\ell_i, f_{i,1}, ..., f_{i,B})_{i = 1}^N\}$, the catalog on the full image. 
