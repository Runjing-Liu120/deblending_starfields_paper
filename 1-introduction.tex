...
When light sources are well separated, catalog construction is straightforward:
...
We then demonstrate the ability of our method to scale to large modern astronomical surveys
...
They avoided the transdimensional latent variable space in order to have a tractable objective for numerical optimization.
...
Finally, and critically, StarNet is fit using a \textit{forward} KL divergence
between the approximate posterior $q$ and the exact posterior $p$,
while traditional variational inference minimizes the a \textit{reverse}
...
However, to the best of our knowledge, this is the first instance of applyiing
variational inference with a foward KL for scientific purposes.
Specifically, we use wake-sleep for inference to find a latent space that is interpretable:
...
and traditional cataloging approaches (Section~\ref{sec:results_on_m2}).
