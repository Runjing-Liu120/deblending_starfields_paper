...
Catalog construction is straightforward when light sources are well separated. Each light source produces a single peak intensity on the image, and characteristics of the light source such as flux can be estimated by analyzing photon counts on the peak and the surrounding pixels. 
...
Third, for blended fields, the properties of light sources are especially ambiguous and therefore, providing calibrated uncertainty quantification is as important as making accurate predictions.
Finally, the scale of the data is immense by any standard. The upcoming Rubin Observatory Legacy Survey of Space and Time (LSST) survey, scheduled to begin data collection in 2022, is expected to produce 60 petabytes of astronomical images over its lifetime~\cite{LSST_about}.
...
is critical for astronomical research where conclusions about the physical universe are derived from estimated catalogs. 
...
Therefore, most of these pipelines are unreliable for highly blended light sources where ambiguity exists in identifying sources and estimating their characteristics.
...
We validated StarNet on the M2 globular cluster. StarNet was more accurate than the MCMC-based cataloger PCAT and traditional cataloging approaches while running $100,000$ times faster than the former (Section~\ref{sec:results_on_m2}).
