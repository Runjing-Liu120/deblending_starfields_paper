...
\begin{wrapfigure}{r}{0.35\textwidth}
    \centering
    \vspace{-1em}
    \includegraphics[width = 0.32\textwidth]{figures/elbo_vs_sleep/vi_sleep_ex_figure.png}
    \vspace{-1.7em}
...
\begin{figure}[!htb]
    ...
    (Bottom row) Modal locations from two variational posteriors in red.
    On the left, a randomly initialized run where optimizing the ELBO with the reparameterized gradient resulted in a shallow local optimum: the estimated locations split a single source into two sources, while missing another source entirely.  
