...
\begin{align}
    \argmin_{\eta} \Expect_{x \sim p(x)}\Big[\KL(p_\phi(z | x) \| q_\eta(z | x)\Big]
    \label{eq:sleep_obj},
\end{align}
...
The loss encourages $q_{\eta}(z | x)$ to map images $x$ to a probability distribution with large mass on the 
catalog $z$.
...
In summary, the sleep phase results in a supervised learning problem on complete data simulated from our generative model. In Section TBD, we will see the benefits in our application of using complete data and optimizing the losses~\eqref{eq:cross_entropy_loss}-\eqref{eq:gaussian_sleep_loss} over optimizing the traditional ELBO. 
