...
of minimizing $\mathcal{L}_{FA}$ \textit{Forward Amortized Inference} (FAVI).
...
There are two key differences between the FAVI objective~\eqref{eq:sleep_obj}
and the ELBO~\eqref{eq:elbo}.
First, recall that maximizing the ELBO is equivalent to minimizing
$\KL(q\|p)$; the KL in the FAVI objective transposes the arguments.
Second, the outer expectation over $p(x)$ also gives different meaning to the FAVI.
...
In contrast, the FAVI objective seeks to minimize the $\KL$ {\itshape on average over all possible data $x$},
as weighted by $p(x)$.
